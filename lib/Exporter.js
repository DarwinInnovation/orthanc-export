...
class Exporter extends EventEmitter {
  ...
  _new_instance(id, details) {
    debug('new instance', id, details.Seq);
            ...
            this._download_instance(instance)
                .then(() => {
                  this._export_instance(instance)
                      .then(() => {
                        debug("Finished "+instance.ID);

                        try {
                          fs.unlinkSync(instance.tmp_path);
                        } catch (e) {
                          console.warn("Failed to unlink ", instance.ID);
                        }
                        ...
                        try {
                          ...
                          console.warn("Failed to save state after ", instance.ID);
                        }
                        resolve();
                      })
                      .catch((err) => {
  ...
  _get_tmp(filename) {
    return path.join(this._tmp_dir, filename);
  }
